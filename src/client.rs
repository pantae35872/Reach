...
use crate::{enums::{S2CCommand, C2SCommand, C2SPacket}, packet::Packet};
...
impl Client {
    ...
    pub async fn main_thread(&mut self, receiver: Arc<Mutex<Receiver<S2CCommand>>>, sender: Arc<Mutex<Sender<C2SCommand>>>) -> ! {
        let mut buffer: Vec<u8> = vec![8; 1024];
            ...
            match stream_opt.as_ref() {
                Some(stream) => {
                    while let Ok(_length) = stream.try_read(&mut buffer) {
                        ...
                        let mut fixed_received_data: [u8; 1024] = [0; 1024];
                        if receive_data.len() <= 1024 {
                            fixed_received_data[..receive_data.len()].copy_from_slice(receive_data);
                        } else if receive_data.len() == 1024 {
                            fixed_received_data.copy_from_slice(&receive_data[..1024]);
                        } else {
                            disconnect = true;
                            break;
                        }
                        
                        let receive_data_clone: [u8; 1024] = fixed_received_data.clone();
